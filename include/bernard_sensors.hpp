...
{
public:
    BernardSensors(Adafruit_BNO055 *imu, uint32_t lFootContactPin, uint32_t rFootContactPin);
    ...
    /// @return A vector of uint32_t containing the analog values from the left and right foot contact sensors.
    std::vector<uint32_t> getFootPressure() { return {footContactLValue, footContactRValue}; }

    /// @brief Timer callback function.
    /// @details This function is called every 100ms to read the IMU data and the foot contact sensors.
    void timerCallback();
